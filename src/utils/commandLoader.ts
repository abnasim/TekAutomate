...
import { parseSCPI } from './scpiParser';
import { detectEditableParameters } from './scpiParameterDetector';
...
export function loadCompleteCommandsFile(data: any): {
  ...
  Object.entries(data.commands_by_section).forEach(([sectionName, sectionCommands]: [string, any]) => {
    ...
    sectionCommands.forEach((cmd: any) => {
      ...
      let editableParams;
      try {
        parsed = parseSCPI(commandStr);
        editableParams = detectEditableParameters(parsed);
      } catch (e) {
        // If parsing fails, continue without structure
        parsed = undefined;
        editableParams = undefined;
      }
