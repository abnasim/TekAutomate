...
export const SCPIParameterSelector: React.FC<SCPIParameterSelectorProps> = ({
  ...
  const getParameterLabel = (param: EditableParameter, idx: number): string => {
    ...
    if (commandParams.length > 0) {
      ...
      const valueParams = commandParams.filter(p => 
        p.name && !['channel', 'math', 'ref', 'bus', 'measurement', 'cursor', 'search', 'power', 'source'].includes(p.name.toLowerCase())
      );
  ...
  return (
    <div className={`${className}`}>
      <div className={`${compact ? 'text-xs' : 'text-sm'} font-semibold mb-2 text-gray-700`}>
        {title}
      </div>
      <div className="space-y-2">
        {enrichedParams.map((param, idx) => {
          ...
          return (
            <div key={idx}>
              <label className={`block ${compact ? 'text-xs' : 'text-sm'} mb-1 text-gray-600 font-medium`}>
                {label}
              ...
              {param.validOptions.length > 0 ? (
                <select
                  ...
                  className={`w-full px-2 ${compact ? 'py-1 text-xs' : 'py-1.5 text-sm'} border rounded bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                >
                ...
                <input
                  ...
                  className={`w-full px-2 ${compact ? 'py-1 text-xs' : 'py-1.5 text-sm'} border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
