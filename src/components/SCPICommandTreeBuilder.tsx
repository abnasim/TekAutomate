...
export const SCPICommandTreeBuilder: React.FC<SCPICommandTreeBuilderProps> = ({
    ...
    for (const [paramName, maxIndex] of Object.entries(PARAM_CONFIGS)) {
      ...
      if (match) {
        ...
        if (matchingCommands.length > 0) {
          ...
          const prefixMatch = matchingCommands[0].scpi.match(prefixExtractRegex);
  ...
  const subCommands = useMemo(() => {
    ...
    const { displayName, matchingCommands, afterParam } = detected;
    const subCmdMap = new Map<string, { name: string, cmd: CommandLibraryItem }>();
    
    const afterParamPattern = new RegExp(`${displayName}<[XN]>:([^:]+)`, 'i');
    
    matchingCommands.forEach(cmd => {
      const match = cmd.scpi.match(afterParamPattern);
      if (match) {
        const subCmdName = match[1];
        if (afterParam) {
          const afterParamUpper = afterParam.replace(/^:/, '').toUpperCase();
          if (!subCmdName.toUpperCase().startsWith(afterParamUpper)) {
            return;
          }
        }
        if (!subCmdMap.has(subCmdName.toUpperCase())) {
          subCmdMap.set(subCmdName.toUpperCase(), { name: subCmdName, cmd });
        }
      }
    });
  ...
  return (
    <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-lg p-3 mt-2 shadow-sm">
      ...
      <div className="flex items-center gap-2 flex-wrap mb-2">
        <span className="text-xs text-gray-600">Select index:</span>
        ...
        <div className="pt-2 border-t border-green-200 bg-green-50 -mx-3 -mb-3 px-3 pb-3 rounded-b-lg">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xs font-semibold text-green-700">
              ✓ Command: {exactMatchCommand.name}
            </span>
          </div>
      ...
      {/* Sub-commands (only show if no exact match) */}
      {!exactMatchCommand && subCommands.length > 0 && (
        <div className="pt-2 border-t border-purple-200">
          <div className="text-xs text-gray-600 mb-2">Sub-commands (click to search):</div>
          <div className="flex flex-wrap gap-1 max-h-32 overflow-y-auto">
            {subCommands.map(({ name, cmd }) => (
              <button
                key={name}
                onClick={() => {
                  onSelectParameter(`${displayName} ${name}`, typedNumber || 1);
                }}
                ...
                title={cmd.description || `Search for ${displayName}<x>:${name} commands`}
              >
